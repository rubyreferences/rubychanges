...
* **Documentation:** — (it is _almost_ documented by refering from [IO.new](https://ruby-doc.org/core-2.5.0/IO.html#method-c-new) docs to [String#encode](https://ruby-doc.org/core-2.5.0/String.html#method-i-encode) docs, where at least longer form `newline_universal: true` is mentioned)
* **Code:**
  ```ruby
...
* **Details:** The constant `TMPFILE` is [documented](https://ruby-doc.org/core-2.5.0/File/Constants.html#TMPFILE) as "Create an unnamed temporary file", but before Ruby 2.5, it was still possible to request this file's path.
...
* [Set#to_s](https://ruby-doc.org/stdlib-2.5.0/libdoc/set/rdoc/Set.html#method-i-to_s) is an alias to `#inspect`. Discussion: [Feature #13676](https://bugs.ruby-lang.org/issues/13676)
  ```ruby
  puts Set[:foo, :bar]
  # Ruby 2.4: #<Set:0x00005614c9069e68>
  # Ruby 2.5: #<Set: {:foo, :bar}>
  ```
* [Set#===](https://ruby-doc.org/stdlib-2.5.0/libdoc/set/rdoc/Set.html#method-i-3D-3D-3D) as alias to `#include?`. Discussion:  [Feature #13801](https://bugs.ruby-lang.org/issues/13801). This allows to use sets in `case` and `grep`:
  ```ruby
  case RUBY_VERSION
