...
* **Details:** When `uplevel: N` provided to `Kernel#warn`, then warning mesage includes file and line of the call, `N` steps up in the callstack.
* **Reason:** It is sometimes hard to understand what exact code caused the warning, this feature helps to show the offencive calling code.
* **Discussion:** <a class="tracker feature" href="https://bugs.ruby-lang.org/issues/12882">Feature #12882</a>
* **Documentation:** <a class="ruby-doc" href="https://ruby-doc.org/core-2.6/Kernel.html#method-i-warn"><code>Kernel#warn</code></a> (it became somehat documented at Ruby 2.6, but the feature itself is available since 2.5)
* **Code:**
  ```ruby
...
* **Details:** `Time.at` second argument was meant to be microseconds, but in 2.5+ it can be micro-, milli- or nano-seconds, specified by third argument: `:millisecond`, `:usec` or `:microsecond` (defaut), `:nanosecond`
...
* **Code:**
  ...
  ```
* **Notice:** The format is different depending of whether **`STDERR`** is TTY or not (even if you printing the returned string to `STDOUT` or just store it in the variable)
...
* `Random.raw_seed` renamed to <a class="ruby-doc" href="https://ruby-doc.org/core-2.5.0/Random.html#method-c-urandom"><code>Random.urandom</code></a>.  It is now applicable to non-seeding purposes due to [Bug #9569].
...
* Set#to_s as alias to #inspect <a class="tracker feature" href="https://bugs.ruby-lang.org/issues/13676">Feature #13676</a>
  ```ruby
  puts Set[:foo, :bar]
  # Ruby 2.4: #<Set:0x00005614c9069e68>
  # Ruby 2.5: #<Set: {:foo, :bar}>
  ```
* Set#=== as alias to `#include?` <a class="tracker feature" href="https://bugs.ruby-lang.org/issues/13801">Feature #13801</a> This allows to use sets in `case` and `grep`:
  ```ruby
  case RUBY_VERSION
  ...
  # => [:open, :close, :open]
  ```
* Set#reset <a class="tracker feature" href="https://bugs.ruby-lang.org/issues/6589">Feature #6589</a> allows to ensure set's internal state consistency after modifying a mutable element of the set:
  ```ruby
...
* `mathn.rb` removed from stdlib. [Feature #10169]

### Large updated[](#large-updated)

* Update to Psych 3.0.2.
* Update to RDoc 6.0.1.
* Update to Rubygems 2.7.3.
* IPAddr https://github.com/ruby/ipaddr/blob/master/CHANGELOG.md#121
* OpenSSL: Updated Ruby/OpenSSL from version 2.0 to 2.1. Changes are noted in "Version 2.1.0" section in ext/openssl/History.md.
