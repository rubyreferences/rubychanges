...
* **Status (as of Dec 27, 2019):** 2.7.0 is current _stable_
...
* Implicit block capturing in `proc` and `lambda`:
  ...
  # still prints "Hello"
  ```
* Contrastingly, the flip-flop syntax deprecation, [introduced](2.6.html#misc) in 2.6, **is reverted**. It turned out that for brevity in text-processing scrips (including one-liners run as `ruby -e "print <something>"`) the feature, however esotheric it may seem, has a justified usage.
...
* **Documentation:** <a class="ruby-doc" href="https://docs.ruby-lang.org/en/master/GC.html#method-c-compact"><code>GC.compact</code></a>
