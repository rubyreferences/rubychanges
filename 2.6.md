...
Source: **[NEWS file](https://github.com/ruby/ruby/blob/trunk/NEWS)**.
...
* **Documentation:** [Range: Endless ranges](http://ruby-doc.org/core-2.6.0.rc2/Range.html#class-Range-label-Endless+Ranges)
...
_Note: here we voluntarily extract changes to "core of core": `Kernel`, atomic data types, callables (`Proc`/`Method`) and enumerables. See next sections for changes to exception classes, filesystem handling and introspection utils._

### `Kernel#then` as an alias for `#yield_self`

* **Reason:** since the introduction of `Kernel#yield_self` at Ruby 2.5, it was pointed that the name choosen is too long for this basic method, and, unlike most of other core methods, says "how it is implemented" not the intention; after lots of discussion it was decided `#then` is the best correspondence for method's goal.
* **Notice:** There is a controversy in the comunity about this alias, pointing out at the fact that `#then` is typical method name for promises.
...
### `Kernel#<Numeric>()` methods have `exception:` argument
...
* **Documentation:** [Kernel#Integer](http://ruby-doc.org/core-2.6.0.rc2/Kernel.html#method-i-Integer), [Kernel#Float](http://ruby-doc.org/core-2.6.0.rc2/Kernel.html#method-i-Float), [Kernel#Rational](http://ruby-doc.org/core-2.6.0.rc2/Kernel.html#method-i-Rational), [Kernel#Complex](http://ruby-doc.org/core-2.6.0.rc2/Kernel.html#method-i-Complex), [Kernel#BigDecimal](http://ruby-doc.org/stdlib-2.6.0.rc2/libdoc/bigdecimal/rdoc/Kernel.html#method-i-BigDecimal)
* **Code:**
  ```ruby
...
### Additional argument to `Module#method_defined?` and similar

`Module#method_defined?` and similar methods accept optional second argument

* **Reason:** Other module introspection methods, like `#methods`, already have similar argument. It may be important for metaprogramming; like "when this module is included, it will redefine some host's methods, but only those that belong to host", or "test that all descendants of some abstract class redefine required methods".
* **Affected methods:** `#method_defined?`, `#private_method_defined?`, `#protected_method_defined?` now accept the second parameter as optional. If it's true (=default), checks ancestor modules/classes, or checks only the class itself.
...
* **Documentation:** [String#split](https://ruby-doc.org/core-2.6.0.rc2/String.html#method-i-split)
* **Code:**
  ```ruby
...
* **Methods affected:**
  ...
  * `#+`, `#-`, `#succ` (no new argument, but presserve timezone of the source)
* **Discussion:** [Feature #14850](https://bugs.ruby-lang.org/issues/14850)
* **Documentation:** [Time: Timezone argument](https://ruby-doc.org/core-2.6.0.rc2/Time.html#class-Time-label-Timezone+argument)
...
* **Code:**
  ...
  ```
* **Important notice**: Unlike any other places in Ruby (where objects are coerced into procs with `#to_proc` method), "what can be chained" is decided by existance of `#call` method; this means you CAN'T chain symbols (notice `:body.to_proc` above), but CAN chain some "command pattern" classes with `MyClass.call` API.
...
* **Documentation:** [Array#union](https://ruby-doc.org/core-2.6.0.rc2/Array.html#method-i-union), [Array#difference](https://ruby-doc.org/core-2.6.0.rc2/Array.html#method-i-difference)
* **Code:**
  ```ruby
  [1, 2, 3].union([3, 4], [4, 5])
  # => [1, 2, 3, 4, 5]
...
* **Documentation:** [Enumerable#to_h](http://ruby-doc.org/core-2.6.0.rc2/Enumerable.html#method-i-to_h), [Array#to_h](http://ruby-doc.org/core-2.6.0.rc2/Array.html#method-i-to_h), [ENV.to_h](http://ruby-doc.org/core-2.6.0.rc2/ENV.html#method-c-to_h), [Hash#to_h](http://ruby-doc.org/core-2.6.0.rc2/Hash.html#method-i-to_h), [Struct#to_h](http://ruby-doc.org/core-2.6.0.rc2/Struct.html#method-i-to_h)
* **Code:**
  ```ruby
...
`Range#step` and `Numeric#step` are now returning not an istance of `Enumerator`, but `Enumerable::ArithmeticSequence` instead.

* **Reason/Usage:** This feature is added by request of scientific Ruby community; it is useful for reusing results of `(from..to).step(s)` as a value obect for idiomatic slicing of custom collections, see also `Range#%`;
* **Discussion:** https://bugs.ruby-lang.org/issues/13904
* **Documentation:** [Enumerator::ArithmeticSequence](https://ruby-doc.org/core-2.6.0.preview2/Enumerator/ArithmeticSequence.html), [Range#step], [Numeric#step]
* **Code:**
  ```ruby
...
* **Reason:** Since introduction of new way for printing exceptions to STDERR (backtrace in reverse order and message is highlighed in bold), there were some improvements proposed to handle different contexts of exception printing.
...
Dir#each_child and Dir#children instance methods. [Feature #13969]
...
* `Random.bytes`: [Feature #14250](https://bugs.ruby-lang.org/issues/14250)
  ```ruby
  Random.bytes(5)
  # => "8\a\xB0\xD1V"
...
The new module provides "official" Ruby parser, replacing stdlib Ripper (and third-party parsers). The module is considered experimental, and its API can change in the future versions.

* **Documentation:** [RubyVM::AbstractSyntaxTree](https://ruby-doc.org/core-2.6.0.rc2/RubyVM/AbstractSyntaxTree.html)
...
* `Kernel#BigDecimal()` method now accepts `exception:` argument (see above for explanations)
* `URI` now can open `file://` URIs, `URI::File` class added.
  * **Documentation:** [URI::File]
  * **Discussion:** [Feature #14035](https://bugs.ruby-lang.org/issues/12733)
...
* **CSV**: [Changelog](https://github.com/ruby/csv/blob/master/NEWS.md).
* **REXML**: [Changelog](https://github.com/ruby/rexml/blob/master/NEWS.md).
* **Matrix**: [Changelog](https://github.com/ruby/matrix/blob/master/CHANGELOG.md#v100--ruby-26).
...
TODO: link to default gems concept
